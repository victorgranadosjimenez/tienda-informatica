...
public class ProductoDAOImpl extends AbstractDAOImpl implements ProductoDAO{
	...
	public List<Producto> getByFilterUsingFullTextIndex(String filter){
		PreparedStatement ps = null;
		Connection conn = null;
		Statement s = null;
		ResultSet rs = null;
		...
		try{
			conn = connectDB();
			rs = s.executeQuery("CREATE FULLTEXT INDEX idx_nombre ON productos(nombre)");
		...
		try {
			...
			ps = conn.prepareStatement("SELECT * FROM productos WHERE MATCH(nombre) AGAINST (?)");
			int idx = 1;
			ps.setString(idx, filter);
			rs = ps.executeQuery();
